...
# genotypes
########Clean bed 1
bed1 = read.plink("NICHD_PolycysticOvary_c1_PCOS.bed", select.snps = name5[,1])
bed1 = bed1$genotypes

bed0 = read.plink("NICHD_PolycysticOvary_c2_NUGENE.bed", select.snps = name5[,1])
bed0 = bed0$genotypes
...
name5 = rownames(snpsum.col)###Update the chromsomes we are looking at. 
...
# Iterative CA
timestamp()
R3LongPrint(data, names = name5, resultFileName = "chrom5LEAN.csv")
timestamp()
