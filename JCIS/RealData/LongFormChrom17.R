...
# genotypes
########Clean bed 1
bed1 = read.plink("NICHD_PolycysticOvary_c1_PCOS.bed", select.snps = name17[,1])
bed1 = bed1$genotypes

bed0 = read.plink("NICHD_PolycysticOvary_c2_NUGENE.bed", select.snps = name17[,1])
bed0 = bed0$genotypes
...
name17 = rownames(snpsum.col)###Update the chromsomes we are looking at. 
...
#smallData = data[,c("Y", name17[1:4000,1])]###Dimensions are off here. 
#dim(smallData)

# Iterative CA
timestamp()
R3LongPrint(data, names = name17, resultFileName = "chrom17LEAN.csv")
timestamp()
